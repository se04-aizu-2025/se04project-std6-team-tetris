...
#include <sys/stat.h>  // ファイル存在チェックに使う

// ファイルが存在するか確認する関数
static bool file_exists(const char *filename) {
    struct stat buffer;
    return (stat(filename, &buffer) == 0);  // 0 → 存在する
}

// 新しい連番ファイル名を作る
static void create_unique_filename(char *out, const char *base) {
    int index = 1;
    do {
        sprintf(out, "%s_%d.json", base, index);
        index++;
    } while (file_exists(out));
}

// 連番付きログファイルを開く start_log
void start_log(const char *basename, const int arr[], int n) {
    g_n = n;

    char filename[256];
    create_unique_filename(filename, basename);
