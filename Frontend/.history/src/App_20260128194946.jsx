...
function App() {
  ...
  // Keep "in-progress" input values (strings) here
  ...
  // Prevent double alert: Enter triggers onKeyDown + onBlur
  const skipNextBlurRef = useRef(false);

  const getNFromTotalInput = () => Number(totallSortingNumber.current?.value ?? "");

  const validateNumber = (raw) => {
    if (raw === "" || raw == null) return { ok: true, value: "" }; // allow empty
    const n = Number(raw);
  ...
  // Validate on Enter / Blur. If invalid: alert + do NOT commit to state.
  ...
  // Manual input: create input fields
  ...
  // Update input while typing (store as digits-only string)
  ...
  // Enter: validate once, move to next field
  const handleKeyDown = (e, index) => {
    if (e.key !== "Enter") return;

    e.preventDefault();

    // Enter often triggers blur next, skip that blur
  ...
  // Save manual inputs to test array
  const saveSelfInputsToTest = () => {
    // validate all first; if any invalid, stop saving
  ...
  // Called from SortVisualizer: generate sorting log
      ...
      <main className="app-main">
        <div className="app-container">
          <div className="sorting-number">
            ...
            {mode === "auto" && (
              <div>
                <p>Type the number of elements (1-{MAX_ARRAY_LENGTH})</p>
                <input
                  type="number"
            ...
            {mode === "self" && (
              <div>
                ...
                <p>* Each value must be between {MIN_VALUE} and {MAX_VALUE}.</p>
        ...
        <div className="app-container app-footer__inner">
          <div className="app-footer__left">
            ...
            <div className="app-footer__subtitle">
              Controls: Next / Back / Auto / Speed to experience the algorithm
            </div>
          </div>
